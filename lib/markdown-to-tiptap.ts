/**
 * Convert markdown into TipTap JSON.
 * Uses `marked.lexer()` for parsing — no DOM required, safe for server-side.
 *
 * Originally written for the Airtable importer's richText fields; now shared so
 * any server-side path that needs to author rich-text content (e.g. the AI
 * agent's CMS collection item tools) produces the exact same Tiptap shape the
